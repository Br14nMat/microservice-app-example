...
locals {
  container_apps = {
    auth_api = {
      name      = "auth-api"
      image     = "${data.azurerm_container_registry.taller1.login_server}/auth-api"
      port      = 8000
      cpu       = 0.5
      memory    = "1Gi"
      min_replicas = 1
    ...
    frontend = {
      name      = "frontend"
      image     = "${data.azurerm_container_registry.taller1.login_server}/frontend"
      port      = 8080
      cpu       = 0.5
      memory    = "1Gi"
      min_replicas = 1
      ...
      env_vars = {
        PORT              = "8080"
        AUTH_API_ADDRESS  = "http://auth-api:8000" # DNS interno
        TODOS_API_ADDRESS = "http://todos-api:8082" # DNS interno
      }
    }
    log_processor = {
      name      = "log-message-processor"
      image     = "${data.azurerm_container_registry.taller1.login_server}/log-message-processor"
    ...
    todos_api = {
      name      = "todos-api"
      image     = "${data.azurerm_container_registry.taller1.login_server}/todos-api"
      port      = 8082
      cpu       = 0.5
      memory    = "1Gi"
      min_replicas = 1
    ...
    users_api = {
      name      = "users-api"
      image     = "${data.azurerm_container_registry.taller1.login_server}/users-api"
      port      = 8083
      cpu       = 0.5
      memory    = "1Gi"
      min_replicas = 1
...
output "auth_api_url" {
  value = "http://${azurerm_container_app.app["auth_api"].ingress[0].fqdn}:8000"
}
